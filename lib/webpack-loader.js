 ...
 * @type {webpack.Loader}
 *
 * Transforms isomorphine.api(rootdir) to isomorphine.api(rootdir, entityMap)
...
module.exports = function(source) {
  ...
  if (source.indexOf('isomorphine.api(') < 0) {
    return source;
  }
  ...
  source = source.replace(/isomorphine\.api\(.*\)/, '__isomorphicAPIFactory()');

  source += '\n' + (
    'function __isomorphicAPIFactory() {\n' +
    '  var __entityMap = ' + JSON.stringify(map) + ';\n' +
    '  return isomorphine.api(null, __entityMap);\n' +
    '}'
  );
...
/**
 * Gets the absolute path to the rootdir argument of the isomorphic.api() call.
 *
...
function getRootdir(context, source) {
  var regexMatch = source.match(/isomorphine.api\((.*)\)/);
  var target = '';
